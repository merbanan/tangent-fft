...
#define LANE2_NEON_PI 3.141592653589793238462643383279502884f
...
struct lane2_neon_plan {
    size_t n;
    size_t inner_size;
    unsigned inner_levels;
    uint32_t *mixed_permutation;
    lane2_neon_root *replicated_root;
    lane2_neon_root *finish_root;
    lane2_neon_row *work;
};

_Static_assert(offsetof(lane2_neon_plan, inner_size) == 8,
               "update lane2 NEON assembly plan offsets");
_Static_assert(offsetof(lane2_neon_plan, inner_levels) == 16,
               "update lane2 NEON assembly plan offsets");
_Static_assert(offsetof(lane2_neon_plan, mixed_permutation) == 24,
               "update lane2 NEON assembly plan offsets");
_Static_assert(offsetof(lane2_neon_plan, replicated_root) == 32,
               "update lane2 NEON assembly plan offsets");
_Static_assert(offsetof(lane2_neon_plan, finish_root) == 40,
               "update lane2 NEON assembly plan offsets");
_Static_assert(offsetof(lane2_neon_plan, work) == 48,
               "update lane2 NEON assembly plan offsets");
_Static_assert(sizeof(lane2_neon_root) == 32,
               "lane2 NEON roots must occupy two vectors");
_Static_assert(sizeof(lane2_neon_row) == 16,
               "lane2 NEON rows must occupy one vector");
...
static void set_replicated_root(lane2_neon_root *destination,
                                float re,
                                float im)
{
    unsigned lane;
...
static void set_packed_root(lane2_neon_root *destination,
                            unsigned lane,
                            float re,
                            float im)
{
...
lane2_neon_plan *lane2_neon_plan_create(size_t n)
{
    ...
    plan = (lane2_neon_plan *)calloc(1, sizeof(*plan));
    if (plan == NULL) {
        return NULL;
    }
    plan->n = n;
    ...
    plan->replicated_root = (lane2_neon_root *)aligned_alloc(
        64,
        round_up(plan->inner_size * sizeof(*plan->replicated_root), 64));
    plan->finish_root = (lane2_neon_root *)aligned_alloc(
        64,
        round_up((plan->inner_size / 2) *
                 sizeof(*plan->finish_root), 64));
    plan->work = (lane2_neon_row *)aligned_alloc(
        64,
